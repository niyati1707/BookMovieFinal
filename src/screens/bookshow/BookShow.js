...
const BookShow = (props) => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8085/api/v1/movies/" + props.location.state.id + "/shows")
  ...
  const bookShowButtonHandler = () => {
    ...
    history.push({
      pathname: "/confirm/" + props.location.state.id,
      bookingSummary: {
        location,
        theatre,
        language,
  ...
  return (
    <div>
      <Header baseUrl={props.location.state.baseUrl} />
      <div className="bookShow">
        <Typography className="back">
            <Link
              to={{
                pathname: `/details/${props.location.state.id}`,
                state: {
                  selectedMovieDetails: props.location.state.item,
                },
              }}
            >
